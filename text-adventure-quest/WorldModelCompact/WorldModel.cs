...
namespace TextAdventures.Quest
{
    ...
    public class WorldModel : ISimulation, IASLDebug, IASLTimer
    {
        ...
        private IController m_playerUI = null;
        ...
        public bool Initialise(IController player, bool? isCompiled = null)
        {
            m_editMode = false;
            m_playerUI = player;
        ...
        public void Begin()
        {
            DoInNewThreadAndWait(() =>
            {
                try
                {
                    ...
                    if (m_elements.ContainsKey(ElementType.Function, "InitInterface"))
                    {
                        RunProcedure("InitInterface");
                    }
                    if (!m_loadedFromSaved)
                    {
                        if (m_elements.ContainsKey(ElementType.Function, "StartGame"))
                        {
        ...
        private string GetExternalPath(string current, string file, bool throwException)
        {
            ...
            if (TryPath(current, file, out path, false))
            {
                return path;
            }
            if (ResourcesFolder == null)
            {
                ...
                if (TryPath(Environment.CurrentDirectory, file, out path, false))
                {
                    return path;
                }
                if (!string.IsNullOrEmpty(m_libFolder) && TryPath(m_libFolder, file, out path, true))
                {
                    return path;
                }
                if (System.Reflection.Assembly.GetEntryAssembly() != null)
                {
                    string entryAssemblyPath = System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetEntryAssembly().CodeBase);
                    if (TryPath(entryAssemblyPath, file, out path, true))
                    {
                        return path;
                    }
                }
            ...
            if (throwException)
            {
                throw new IncludeFileMissingException(file, string.Format("Cannot find a file called '{0}' in current path or application/resource path", file));
            }
            return null;
        }
        ...
        {
            path = Utility.RemoveFileColonPrefix(path);
        ...
        public string GameID
        {
            get
            {
                ...
                return Utility.FileMD5Hash(m_filename);
            }
        }
        ...
        public static Func<string, XmlReader> BackupReferenceReader { get; set; }

        internal static System.Xml.XmlReader CreateXmlReader(string elementLabel)
        {
            if (BackupReferenceReader != null)
            {
                return BackupReferenceReader(elementLabel);
            }
            return null;
        }
    }
